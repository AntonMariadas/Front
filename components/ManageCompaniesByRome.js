...
const ManageCompaniesByRom = ({ label }) => {
  const nodeJsApi = process.env.NEXT_PUBLIC_NODEJS_API;
  const [romeCompanies, setRomeCompanies] = useState([]);
  const [rerender, setRerender] = useState(false);

  useEffect(() => {
    getCompaniesByRome(label);
  }, [rerender]);

  const getCompaniesByRome = (label) => {
    axios.get(`${nodeJsApi}/api/rome/${label}`)
      .then(res => setRomeCompanies(res.data))
      .catch(err => console.log(err));
  };

  const deleteCompany = (id) => {
    axios.delete(`${nodeJsApi}/api/companies/`);
  };
  ...
  return (
    <div className="table-responsive">
      <table className="table table-hover">
        ...
        <tbody>
          {romeCompanies.map((company, index) => {
            return (
...
export default ManageCompaniesByRom;